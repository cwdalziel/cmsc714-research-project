...
// Topology-agnostic 3D 7-point stencil.
// No topology-specific rank remapping or pinned decomposition policy.
// Usage: prog [N] | prog NX NY NZ   (default N = 512 cubic if no args)
...
int main(int argc, char* argv[]) {
    ...
    const int local_nx = NX / dims[0];
    const int local_ny = NY / dims[1];
    const int local_nz = NZ / dims[2];
    const int x_start = coords[0] * local_nx;
    const int y_start = coords[1] * local_ny;
    const int z_start = coords[2] * local_nz;

    const int halo_nx = local_nx + 2;
    const int halo_ny = local_ny + 2;
    const int halo_nz = local_nz + 2;
    ...
    for (int x = 1; x <= local_nx; ++x) {
        for (int y = 1; y <= local_ny; ++y) {
            for (int z = 1; z <= local_nz; ++z) {
                grid[idx(x, y, z)] =
                    static_cast<double>((x_start + x - 1) + (y_start + y - 1) + (z_start + z - 1));
            }
        }
    }

    const int yz_plane = local_ny * local_nz;
    const int xz_plane = local_nx * local_nz;
    const int xy_plane = local_nx * local_ny;
